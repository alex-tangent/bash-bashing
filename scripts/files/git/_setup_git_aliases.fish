...
# How to use:
#  run _setup_git_aliases
#  required addition to config.fish:
#   Source all git alias functions
#   for file in /mnt/c/Users/recra/.config/fish/functions/git_aliases/*.fish
#      source $file
#   end

set -l functions_dir ~/.config/fish/functions/git_aliases
...
echo 'function gpushup; git branch | grep \'*\' | sed \'s/* /head:zp\\//\' | xargs git push up -u; end' > $functions_dir/gpushup.fish
echo 'function gdelup; git branch | grep \'*\' | sed \'s/* /zp\\//\' | xargs git push up --delete; end' > $functions_dir/gdelup.fish
echo 'function gdel; git branch -D; end' > $functions_dir/gdel.fish
echo 'function gmaster; git move main upstream/main; end' > $functions_dir/gmain.fish
...
# Now append the necessary lines to config.fish
#set -l config_file ~/.config/fish/config.fish

# Append a new line to config.fish
#echo "" >> $config_file

# Append a comment
#echo "# Source all git alias functions" >> $config_file

# Append the sourcing code
#echo "for file in $functions_dir/*.fish" >> $config_file
